...
import DnsRecordSet from '/src/assets/Network/DNS/dns-record-set.png'
...
<Image src={DnsRecordSet} alt="Azure DNS Record Set Screen" width="750" />
For example, suppose the 'www.contoso.com' web site is hosted on two different IP addresses. The website requires two different A records, one for each IP address. This example shows a simple example DNS record set:
