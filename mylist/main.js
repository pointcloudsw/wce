...
(async function() {
  const fetchcss = await fetch('./mylist.css');
  const mycss = await fetchcss.text();
  const fetchhtml = await fetch('./content.txt');
  const myhtml = await fetchhtml.text();
  class MyList extends HTMLElement {
    constructor() {
      ...
      myListContainer.classList.add('editable-list');
      ...
      shadow.appendChild(myListContainer);
    }
